...
class Finance extends MY_Controller
{
        ...
        else
        {
            ...
            if ($transection)
            {
                ...
                if ($acc_id != base_account()->acc_id)
                {
                    if ($this->input->post('tr_status') == 2)
                    {
                        $base_acc_new_amount = base_account()->acc_amount - $this->input->post('tr_amount');
                    }
                    else
                    {
                        if ($acc_info->acc_type == 1)
                            $base_acc_new_amount = base_account()->acc_amount - $this->input->post('tr_amount');
                        else
                            $base_acc_new_amount = base_account()->acc_amount + $this->input->post('tr_amount');
                    ...
                    $this->finance_model->data_save(['acc_amount' => $base_acc_new_amount], base_account()->acc_id);
                }
    ...
    public function delete_transection($tr_id, $acc_id, $acc_amount)
    {
        ...
        $this->finance_model->accounts();
        $account_info = $this->finance_model->data_get($acc_id, TRUE);
        
        if ($transection->tr_status == 1)
        {
            if ($account_info->acc_type == 1)
                $base_acc_new_amount = base_account()->acc_amount + $transection->tr_amount;
            else
                $base_acc_new_amount = base_account()->acc_amount - $transection->tr_amount;
    ...
    public function insert_stock_expence()
    {
        ...
        if (isset($data['first']))
        {
            ...
            $this->session->set_userdata('total_amount', $data['dex_total_amount']);
            // insert DEX
            $data_dex = array(
        ...
        else
        {
            ...
            $tr_amount   = $transection->tr_amount + 0;
            ...
            $new_total_amount = $this->session->total_amount + $data['dex_total_amount'];
    ...
    public function insert_stock_pay_transection()
    {
        ...
        if (is_int($tr_id))
        {
            ...
            $this->finance_model->data_save(['acc_amount' => base_account()->acc_amount - $data['tr_amount']], base_account()->acc_id);
            $this->session->set_flashdata('form_success', 'عملیات با موفقیت انجام شد.');
            $this->end_buy();
        }
        else
